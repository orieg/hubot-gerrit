...
module.exports = (robot) ->
  ...
  gerrit.user = gerrit.auth.split(':')[0]
...
searchMe = (robot, gerrit) -> (msg) ->
  cp.exec "ssh #{gerrit.hostname} -l #{gerrit.user} -p #{gerrit.port} gerrit query --format=JSON -- #{msg.match[1]}", (err, stdout, stderr) ->
    if err
...
eventStreamMe = (robot, gerrit) ->
  ...
  streamEvents = cp.spawn "ssh", [gerrit.hostname, "-l", gerrit.user, "-p", gerrit.port, "gerrit", "stream-events"]
  done = false
  reconnect = null
